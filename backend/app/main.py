...
async def serve_frontend_root():
    ...
    return {"app":"SERVIYA.do API 🇩🇴","tagline":"Trabajo • Confianza • Oportunidades","status":"online","docs":"/docs"}

@app.get("/{full_path:path}",include_in_schema=False)
async def serve_frontend_spa(full_path:str):
    if full_path.startswith("api/") or full_path.startswith("docs") or full_path.startswith("openapi.json"): raise HTTPException(status_code=404,detail="Not found")
    target_file=DIST_DIR/full_path
    if target_file.is_file(): return FileResponse(str(target_file))
    if INDEX_FILE.exists(): return FileResponse(str(INDEX_FILE))
    alt_index=Path.cwd()/"dist"/"index.html"
    if alt_index.exists(): return FileResponse(str(alt_index))
    return {"app":"SERVIYA.do API 🇩🇴","tagline":"Trabajo • Confianza • Oportunidades","status":"online","docs":"/docs"}
