...
def _process_completed(withdrawal_id: int, admin_user: User, db: Session):
    ...
    movement_ref = f"WITHDRAWAL-PROCESSED-{withdrawal_id}"
    db.execute(text("INSERT INTO financial_movements (wallet_id,reference,movement_type,amount_dop,description,created_at) SELECT :wallet_id,:ref,'RETIRO_PROCESADO',:amount,:description,CURRENT_TIMESTAMP WHERE NOT EXISTS (SELECT 1 FROM financial_movements WHERE reference=:ref)"), {"wallet_id": wallet["id"], "ref": movement_ref, "amount": -amount, "description": f"Retiro {withdrawal_id} confirmado por Administración."})
...
def update_withdrawal_status(withdrawal_id: int, data: WithdrawalStatusBody, admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    movement_ref = f"WITHDRAWAL-REJECTED-{withdrawal_id}"
    db.execute(text("INSERT INTO financial_movements (wallet_id,reference,movement_type,amount_dop,description,created_at) SELECT :wallet_id,:ref,'RETIRO_RECHAZADO',:amount,:description,CURRENT_TIMESTAMP WHERE NOT EXISTS (SELECT 1 FROM financial_movements WHERE reference=:ref)"), {"wallet_id": wallet["id"], "ref": movement_ref, "amount": amount, "description": f"Saldo devuelto por retiro rechazado {withdrawal_id}."})
