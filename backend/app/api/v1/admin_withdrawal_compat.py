...
def _process_completed(withdrawal_id: int, admin_user: User, db: Session):
    ...
    # Procesar el retiro NO debe volver a tocar pending_custody_balance.
    ...
    db.execute(text("INSERT INTO transactions (user_id,amount,type,status,reference_code,created_at) SELECT :uid,:amount,'RETIRO_PROCESADO','COMPLETADO',:ref,CURRENT_TIMESTAMP WHERE NOT EXISTS (SELECT 1 FROM transactions WHERE user_id=:uid AND reference_code=:ref)"), {"uid": row["worker_id"], "amount": amount, "ref": f"WITHDRAWAL-{withdrawal_id}"})
