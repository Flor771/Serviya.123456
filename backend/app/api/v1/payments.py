...
from app.models.models import Service, Escrow, Wallet, ServiceStatusEnum, User, BankAccount

router = APIRouter(prefix="/payments", tags=["Pagos y Custodia (Escrow)"])
class EscrowPaymentSchema(BaseModel): service_id: str
class BankTransferEscrowSchema(BaseModel):
    service_id: str
...
def _tx(db,u,a,t,s,d,r, voucher=None, bank_account_id=None):
    db.execute(text("INSERT INTO transactions (user_id,amount,type,status,reference_code,created_at,voucher_url,bank_account_id) VALUES (:u,:a,:t,:s,:r,CURRENT_TIMESTAMP,:v,:b)"),{"u":u,"a":a,"t":t,"s":s,"r":r,"v":voucher,"b":bank_account_id})
...
def _validate_voucher(voucher):
    if not voucher or not voucher.startswith("data:image/") or len(voucher) > 900000:
        raise HTTPException(400,"Debes subir el recibo/voucher del depósito (imagen JPG, PNG o WebP, máximo 900 KB).")
...
def pay_escrow(data:EscrowPaymentSchema,current_user:User=Depends(get_current_active_user),db:Session=Depends(get_db)):
    ...
    db.add(escrow); db.execute(text("UPDATE client_wallets SET available_balance=available_balance-:a,total_spent=total_spent+:a,updated_at=CURRENT_TIMESTAMP WHERE id=:id"),{"a":amount,"id":wallet["id"]}); service.status=ServiceStatusEnum.EN_PROGRESO
    ref=f"ESCROW-{uuid.uuid4().hex[:8].upper()}"; _tx(db,current_user.id,amount,"PAGO_CUSTODIA","RETENIDO",f"Pago en Custodia para servicio #{service.id[:8]}",ref); db.commit(); db.refresh(escrow)
...
def bank_transfer_escrow(data:BankTransferEscrowSchema,current_user:User=Depends(get_current_active_user),db:Session=Depends(get_db)):
    ...
    account=None
    if data.bank_account_id is not None: account=db.query(BankAccount).filter(BankAccount.id==data.bank_account_id,BankAccount.is_active==True).first()
    ...
    existing=db.query(Escrow).filter(Escrow.service_id==service.id,Escrow.status.in_(["RETENIDO","EN_DISPUTA"])).first()
    if existing: raise HTTPException(400,"Este servicio ya tiene un depósito en Custodia SERVIYA.")
    amount=float(service.price_rd)
    if amount<=0: raise HTTPException(400,"El monto del servicio debe ser mayor que RD$ 0.")
    wallet=_client_wallet(db,current_user.id); rate=float(settings.PLATFORM_COMMISSION_PERCENT); commission=amount*rate/100; payout=amount-commission
    ...
    _tx(db,current_user.id,amount,"PAGO_CUSTODIA_TRANSFERENCIA","RETENIDO",f"Depósito bancario con voucher para servicio #{service.id[:8]}",ref,data.voucher_url,data.bank_account_id)
    if service.worker_id:
        db.add(__import__('app.models.models',fromlist=['Notification']).Notification(user_id=service.worker_id,title="Depósito en Custodia SERVIYA",message=f"El cliente realizó el depósito de RD$ {amount:,.2f} y adjuntó el voucher. Ya puedes coordinar el trabajo.",type="PAYMENT",related_entity_id=service.id))
    db.commit(); db.refresh(escrow)
    return {"message":"Voucher recibido y fondos registrados en Custodia SERVIYA.","escrow_id":escrow.id,"reference":ref,"status":"RETENIDO","voucher_received":True}
...
def refund_escrow(data:RefundSchema,current_user:User=Depends(get_current_active_user),db:Session=Depends(get_db)):
    ...
    db.execute(text("UPDATE client_wallets SET available_balance=available_balance+:a,total_refunded=total_refunded+:a,updated_at=CURRENT_TIMESTAMP WHERE id=:id"),{"a":amount,"id":wallet["id"]}); ref=f"REFUND-{uuid.uuid4().hex[:8].upper()}"; _tx(db,current_user.id,amount,"REEMBOLSO","EXITOSO",f"Reembolso del servicio #{data.service_id[:8]}: {data.reason}",ref); db.commit()
    balance=db.execute(text("SELECT available_balance FROM client_wallets WHERE id=:id"),{"id":wallet["id"]}).scalar_one(); return {"message":f"Reembolso de RD$ {amount:,.2f} acreditado a la Billetera SERVIYA.","refund_amount_rd":amount,"reference":ref,"wallet_available_rd":float(balance)}
