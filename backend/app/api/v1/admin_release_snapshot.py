...
def approve_release_snapshot(service_id: str, data: ReleaseApproval, admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    escrow = db.execute(text("SELECT id,client_id,worker_id,total_amount_rd,commission_amount_rd,worker_payout_rd,gross_service_amount_rd,platform_commission_rd,isr_withheld_rd,itbis_withheld_rd,net_worker_payout_rd,fiscal_rule_code,tax_mode,status FROM escrows WHERE service_id=:sid AND status='PENDIENTE_APROBACION' ORDER BY created_at DESC LIMIT 1 FOR UPDATE"), {"sid": service_id}).mappings().first()
    if not escrow:
    ...
    isr = float(escrow["isr_withheld_rd"] or 0); itbis = float(escrow["itbis_withheld_rd"] or 0)
    net = float(escrow["net_worker_payout_rd"] if escrow["net_worker_payout_rd"] is not None else (escrow["worker_payout_rd"] or 0))
    if any(v < 0 for v in (gross, commission, isr, itbis, net)): raise HTTPException(409, "El snapshot financiero contiene valores negativos y no puede liquidarse.")
    if commission > gross: raise HTTPException(409, "La comisión histórica supera el monto bruto de la custodia.")
    if float(wallet["pending_custody_balance"] or 0) + 0.01 < gross: raise HTTPException(409, "El saldo de Custodia del trabajador no cubre el monto histórico de esta liberación.")
    if has_snapshot:
        if abs((commission + isr + itbis + net) - gross) > 0.01: raise HTTPException(409, "El snapshot fiscal no cuadra: bruto, comisión, retenciones y neto no coinciden.")
    elif abs((commission + net) - gross) > 0.01: raise HTTPException(409, "La liquidación histórica no cuadra: comisión y payout no coinciden con el bruto.")
    now = datetime.utcnow(); release_ref = f"ADMIN-RELEASE-{service_id[:8].upper()}"
    result = db.execute(text("UPDATE escrows SET status='LIBERADO', released_at=:now, commission_amount_rd=:commission, worker_payout_rd=:net WHERE id=:id AND status='PENDIENTE_APROBACION'"), {"id": escrow["id"], "now": now, "commission": commission, "net": net})
    if result.rowcount != 1:
        raise HTTPException(409, "La liberación cambió de estado antes de completarse; no se aplicaron fondos.")
    db.execute(text("UPDATE wallets SET available_balance=COALESCE(available_balance,0)+:net,pending_custody_balance=COALESCE(pending_custody_balance,0)-:gross,total_earnings=COALESCE(total_earnings,0)+:net,total_commissions=COALESCE(total_commissions,0)+:commission WHERE id=:id"), {"id": wallet["id"], "net": net, "gross": gross, "commission": commission})
    db.execute(text("INSERT INTO wallet_transactions (id,wallet_id,user_id,type,amount_rd,description,reference,status,created_at) SELECT :id,:wid,:u,'LIBERACION_ADMIN',:net,:description,:ref,'EXITOSO',CURRENT_TIMESTAMP WHERE NOT EXISTS (SELECT 1 FROM wallet_transactions WHERE reference=:ref)"), {"id": str(uuid.uuid4()), "wid": wallet["id"], "u": escrow["worker_id"], "net": net, "description": f"Liberación administrativa del servicio {service_id}", "ref": release_ref})
    db.execute(text("INSERT INTO transactions (user_id,amount,type,status,reference_code,created_at) SELECT :u,:net,'LIBERACION_ADMIN','COMPLETADO',:ref,CURRENT_TIMESTAMP WHERE NOT EXISTS (SELECT 1 FROM transactions WHERE user_id=:u AND reference_code=:ref)"), {"u": escrow["worker_id"], "net": net, "ref": release_ref})
    ...
    if not warranty:
        ...
        db.execute(text("INSERT INTO service_warranties (id,service_id,client_id,worker_id,coverage_days,status,activated_at,expires_at,certificate_ref) VALUES (:id,:sid,:c,:w,60,'ACTIVA',:now,:exp,:ref)"), {"id": str(uuid.uuid4()), "sid": service_id, "c": escrow["client_id"], "w": escrow["worker_id"], "now": now, "exp": expires, "ref": f"GAR-SRV-{service_id[:8].upper()}"})
    fiscal_rule = escrow["fiscal_rule_code"] or "PENDIENTE_CLASIFICACION"; tax_mode = escrow["tax_mode"] or "CONFIGURACION"
    ...
    db.commit()
    return {"message": "Fondos liberados correctamente usando el snapshot histórico de Custodia.","status": "LIBERADO","worker_payout_rd": net,"commission_rd": commission,"isr_withheld_rd": isr,"itbis_withheld_rd": itbis,"fiscal_rule_code": fiscal_rule,"tax_mode": tax_mode,"service_status": "COMPLETADA","reference": release_ref}