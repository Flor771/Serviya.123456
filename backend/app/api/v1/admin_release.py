...
def _audit(db, admin_id, action, service_id, notes):
    db.execute(text("INSERT INTO admin_audit_logs (admin_id,action,resource,target_id,details,timestamp) VALUES (:a,:action,'escrows',NULL,:details,CURRENT_TIMESTAMP)"), {"a":admin_id,"action":action,"details":f"service_id={service_id}; {notes or 'Aprobación administrativa'}"})
...
def approve_release(service_id:str,data:ReleaseApproval,admin_user:User=Depends(require_admin),db:Session=Depends(get_db)):
    ...
    if not worker_wallet:
        db.execute(text("INSERT INTO wallets (worker_id,available_balance,pending_custody_balance,total_earnings,total_commissions,total_withdrawn) VALUES (:w,0,0,0,0,0)"),{"w":escrow['worker_id']}); worker_wallet=db.execute(text("SELECT id FROM wallets WHERE worker_id=:w FOR UPDATE"),{"w":escrow['worker_id']}).mappings().one()
    ...
    _audit(db,admin_user.id,'ADMIN_APPROVE_RELEASE',service_id,data.notes); db.commit()
