...
def pending_releases(admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    for r in rows:
        item=dict(r)
        item["amount_rd"]=float(r["total_amount_rd"] or 0)
        item["payout_rd"]=float(r["worker_payout_rd"] or 0)
        item["commission_rd"]=float(r["commission_amount_rd"] or 0)
        item["display_title"]=f"{r['client_name'] or 'Cliente'} — {r['title'] or 'Servicio'}"
        items.append(item)
