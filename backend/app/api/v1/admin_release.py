...
def pending_deposits(admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    for r in rows:
        item=dict(r)
        item["display_title"] = f"{item.get('client_name') or item.get('client_email') or 'Cliente'} — {item.get('title') or 'Servicio'}"
        deposits.append(item)
    pending_total = sum(float(r["total_amount_rd"] or 0) for r in rows)
    return {"pending_deposits": deposits, "summary": {"count": len(deposits), "pending_total_rd": pending_total}}
...
def deposit_ledger(admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    return {"deposits":[dict(r) for r in rows],"summary":{"count":len(rows),"total_deposited_rd":total,"pending_verification_rd":pending,"held_custody_rd":held,"released_rd":released}}
...
def approve_release(service_id: str, data: ReleaseApproval, admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    if not warranty:
        expires=now+timedelta(days=60); ref=f"GAR-SRV-{service_id[:8].upper()}"
