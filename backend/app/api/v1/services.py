...
def create_service(
    ...
    service = Service(
        ...
        price_rd=data.price_rd,
        province=data.province,
        municipality=data.municipality,
        address_approx=data.address_approx,
    ...
    return {
        "message": "Servicio publicado exitosamente en SERVIYA.do",
        "service": {
            "id": service.id,
            "title": service.title,
...
def get_service_applications(id: str, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    for a in apps:
        ...
        results.append({
            ...
            "worker_name": f"{worker.first_name} {worker.last_name}" if worker else "Técnico SERVIYA",
