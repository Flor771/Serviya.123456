...
def create_review(
    ...
    duplicate = db.execute(text("""
        ...
        WHERE service_id = :sid AND reviewer_id = :reviewer
        LIMIT 1
    """), {"sid": data.service_id, "reviewer": current_user.id}).scalar()
    if duplicate:
        raise HTTPException(status_code=409, detail="Ya calificaste este servicio")

    review_id = db.execute(text("""
        INSERT INTO reviews
            (contract_id, service_id, reviewer_id, reviewee_id, target_user_id, rating, comment, created_at)
        VALUES (NULL, :sid, :reviewer, :target, :target, :rating, :comment, :created_at)
        RETURNING id
    """), {
        "sid": data.service_id,
    ...
    db.execute(text("UPDATE users SET rating = :rating WHERE id = :uid"), {
        "rating": round(float(avg[0]), 2),
    ...
    return {
        ...
        "review": {"id": review_id, "rating": data.rating, "comment": data.comment}
    }
