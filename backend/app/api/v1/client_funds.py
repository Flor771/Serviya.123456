...
def get_client_funds(current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    rows = db.query("""SELECT e.id AS escrow_id, e.service_id, e.worker_id,
    ...
    rows = db.execute(rows, {"client_id": current_user.id}).mappings().all()
    ...
    for row in rows:
        ...
        funds.append({
            ...
            "reference": f"ESCROW-{str(row['escrow_id']).upper()}",
            "date": row["created_at"].isoformat() if row["created_at"] else None,
            "contract_id": None,
            "job_id": str(row["service_id"]),
        })

    total = sum(item["amount_dop"] for item in funds if item["status"] != "REEMBOLSADO")
