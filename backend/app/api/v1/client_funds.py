...
def get_client_funds(current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    """), {"client_id": current_user.id}).mappings().all()

    labels = {
        "PENDIENTE_VERIFICACION": "Voucher recibido — pendiente de verificación",
        "RETENIDO": "En Custodia SERVIYA",
        "PENDIENTE_APROBACION": "Pendiente de liberación administrativa",
    ...
    total = sum(x["amount_dop"] for x in funds if x["status"] != "RECHAZADO")
