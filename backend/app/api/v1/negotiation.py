...
def offer_price(service_id: str, data: PriceOfferSchema, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    if s["status"] not in ("TRABAJADOR_SELECCIONADO", "EN_PROGRESO"):
        raise HTTPException(400, "La negociación de precio está disponible después de seleccionar al trabajador")
    active_escrow = db.execute(text("SELECT id FROM escrows WHERE service_id=:sid AND status IN ('RETENIDO','EN_DISPUTA') LIMIT 1"), {"sid": service_id}).scalar()
    if active_escrow: raise HTTPException(400, "El precio ya quedó financiado en Custodia SERVIYA y no puede modificarse desde la negociación")
...
def accept_price(service_id: str, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    active_escrow = db.execute(text("SELECT id FROM escrows WHERE service_id=:sid AND status IN ('RETENIDO','EN_DISPUTA') LIMIT 1"), {"sid": service_id}).scalar()
    if active_escrow: raise HTTPException(400, "El servicio ya tiene fondos en Custodia SERVIYA")
    ...
    notify(db, s["client_id"] if current_user.id == s["worker_id"] else s["worker_id"], "Precio acordado", f"Precio acordado para {s['title']}: RD$ {agreed:,.2f}. El cliente ya puede proceder al pago en Custodia SERVIYA.", "PRICE_AGREED", service_id)
    db.commit()
