...
def get_negotiation(service_id: str, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    if current_user.id not in {s["client_id"], s["worker_id"]}:
        raise HTTPException(403, "Solo el cliente y el técnico asignado pueden negociar este servicio")
...
def offer_price(service_id: str, data: PriceOfferSchema, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    if not s:
        raise HTTPException(404, "Servicio no encontrado")
    if current_user.id not in {s["client_id"], s["worker_id"]}:
        raise HTTPException(403, "No perteneces a esta negociación")
    if not s["worker_id"]:
        raise HTTPException(400, "Primero debe seleccionarse un trabajador")
    st = s["status"].value if hasattr(s["status"], "value") else str(s["status"])
    if st not in ("TRABAJADOR_SELECCIONADO", "EN_PROGRESO"):
    ...
    if active:
        raise HTTPException(400, "El precio ya quedó financiado en Custodia SERVIYA y no puede modificarse")
...
def accept_price(service_id: str, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    if current_user.id not in {s["client_id"], s["worker_id"]}:
        raise HTTPException(403, "No perteneces a esta negociación")
    if not s["negotiation_offer_rd"] or not s["negotiation_offer_by"]:
        raise HTTPException(400, "Todavía no existe una propuesta de precio para aceptar")
    if s["negotiation_offer_by"] == current_user.id:
        raise HTTPException(400, "La misma persona que propuso el precio no puede aceptarlo")
    active = db.execute(text("SELECT id FROM escrows WHERE service_id=:sid AND status IN ('RETENIDO','EN_DISPUTA') LIMIT 1"), {"sid":service_id}).scalar()
    if active:
        raise HTTPException(400, "El servicio ya tiene fondos en Custodia SERVIYA")
