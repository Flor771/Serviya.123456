...
def get_kpis(admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    return {"stats": {
        ...
        "escrow_held_rd": float(escrow_held_rd),
...
def get_admin_users(admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    return {"users": [{
        ...
        "is_verified": u.is_verified, "province": u.province, "created_at": str(u.created_at)
    } for u in users]}
...
def update_user_status(id: str, status: str = Body(..., embed=True), admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    # Production users has no status column. Keep this action auditable without
    # inventing a new user-state column or changing the existing UI contract.
    _audit(db, admin_user.id, "USER_STATUS_UPDATED", "users", 0, f"Usuario {id} marcado como {status}")
    db.commit()
    return {"message": f"Estado del usuario registrado como {status}"}
...
def process_withdrawal(id: int, admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    row = db.execute(text("""
        SELECT id, worker_id, amount, status FROM withdrawals WHERE id = :id FOR UPDATE
    ...
    wallet = db.execute(text("""
        SELECT id, pending_custody_balance, total_withdrawn
        FROM wallets WHERE worker_id = :uid FOR UPDATE
    """), {"uid": row["worker_id"]}).mappings().first()
    if not wallet:
        raise HTTPException(status_code=400, detail="El trabajador no tiene billetera")

    amount = float(row["amount"])
    pending = float(wallet["pending_custody_balance"] or 0)
    db.execute(text("UPDATE withdrawals SET status = 'COMPLETADO' WHERE id = :id"), {"id": id})
    db.execute(text("""
        UPDATE wallets
        SET pending_custody_balance = GREATEST(0, pending_custody_balance - :amount),
            total_withdrawn = COALESCE(total_withdrawn, 0) + :amount
        WHERE id = :wallet_id
    """), {"amount": amount, "wallet_id": wallet["id"]})
    ...
    db.execute(text("""
        ...
        VALUES (:nid, :uid, 'Retiro procesado', 'Tu retiro fue marcado como completado.', 'RETIRO', false, :related, :created_at)
    """), {"nid": uuid4().hex, "uid": row["worker_id"], "related": str(id), "created_at": datetime.utcnow()})
    db.commit()
...
def resolve_dispute(id: int, data: ResolveDisputeSchema, admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    disp = db.execute(text("""
        SELECT id, service_id, opened_by_user_id, against_user_id, status
    ...
    escrow = db.execute(text("""
        SELECT id, client_id, worker_id, total_amount_rd, commission_amount_rd, worker_payout_rd, status
        FROM escrows WHERE service_id = :sid AND status IN ('RETENIDO','EN_DISPUTA')
        ORDER BY created_at DESC LIMIT 1 FOR UPDATE
    """), {"sid": disp["service_id"]}).mappings().first()
    if not escrow:
    ...
    if data.action == "REFUND_TO_CLIENT":
        ...
        db.execute(text("""
            UPDATE wallets
            SET available_balance = COALESCE(available_balance,0) + :payout,
                total_earnings = COALESCE(total_earnings,0) + :payout,
                total_commissions = COALESCE(total_commissions,0) + :commission
            WHERE id = :wid
        """), {"wid": worker_wallet["id"], "payout": float(escrow["worker_payout_rd"]), "commission": float(escrow["commission_amount_rd"])})
    ...
    db.execute(text("""
        UPDATE disputes SET status = 'RESUELTA', admin_notes = :notes, resolution_notes = :notes WHERE id = :id
    """), {"id": id, "notes": data.resolution})
    ...
    for uid in {disp["opened_by_user_id"], disp["against_user_id"]}:
        if uid:
            db.execute(text("""
                INSERT INTO notifications (id, user_id, title, message, type, read, related_entity_id, created_at)
                VALUES (:nid, :uid, 'Disputa resuelta', :message, 'DISPUTA', false, :related, :created_at)
            """), {"nid": uuid4().hex, "uid": uid, "message": message, "related": str(id), "created_at": datetime.utcnow()})
    db.commit()
...
def update_settings(data: UpdateSettingsSchema, admin_user: User = Depends(require_admin)):
    ...
    return {"message": "Configuración de plataforma actualizada exitosamente.", "settings": {
...
def get_audit_logs(admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    rows = db.execute(text("""
        ...
        FROM admin_audit_logs ORDER BY timestamp DESC NULLS LAST, id DESC
    """)).mappings().all()
    return {"audit_logs": [{
        "id": r["id"], "user_id": r["admin_id"], "admin_id": r["admin_id"],
        "action": r["action"], "resource": r["resource"], "target_id": r["target_id"],
        "details": r["details"], "created_at": str(r["timestamp"])
    } for r in rows]}
...
def get_admin_bank_accounts(admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    return {"bank_accounts": [{
        ...
        "rnc_cedula": a.rnc_cedula, "is_active": a.is_active if a.is_active is not None else True,
...
def create_admin_bank_account(data: BankAccountSchema, admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    account = BankAccount(
        bank_name=data.bank_name, account_number=data.account_number, account_type=data.account_type,
        account_holder=data.account_holder, rnc_cedula=data.rnc_cedula,
    ...
    return {"message": "Cuenta bancaria agregada exitosamente.", "bank_account": {
        ...
        "account_type": account.account_type, "account_holder": account.account_holder,
        "rnc_cedula": account.rnc_cedula, "is_active": account.is_active, "is_primary": account.is_primary
    }}
