...
def _notify(db, user_id, title, message, kind):
    if user_id:
        db.execute(text("INSERT INTO notifications (user_id,title,message,type,read,created_at) VALUES (:u,:t,:m,:k,false,CURRENT_TIMESTAMP)"), {"u":user_id,"t":title,"m":message,"k":kind})
