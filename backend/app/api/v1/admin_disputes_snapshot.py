...
def resolve_dispute_snapshot(dispute_id: str, data: DisputeResolution, admin_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    if resolution == "TRABAJADOR":
        ...
        if result.rowcount != 1:
            db.rollback()
            raise HTTPException(409, "La Custodia cambió de estado antes de liquidarse; no se aplicaron fondos.")
        ...
        if result.rowcount != 1:
            db.rollback()
            raise HTTPException(409, "La Custodia cambió de estado antes de reembolsarse; no se aplicaron fondos.")
