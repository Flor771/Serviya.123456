...
class Escrow(Base):
 __tablename__="escrows"; id=Column(String,primary_key=True,default=generate_uuid); service_id=Column(String,ForeignKey("services.id"),nullable=False); client_id=Column(String,ForeignKey("users.id"),nullable=False); worker_id=Column(String,ForeignKey("users.id"),nullable=False); total_amount_rd=Column(Float,nullable=False); commission_rate_percent=Column(Float,default=10.0); commission_amount_rd=Column(Float,nullable=False); worker_payout_rd=Column(Float,nullable=False); gross_service_amount_rd=Column(Float); platform_commission_rd=Column(Float); isr_withheld_rd=Column(Float,default=0.0); itbis_withheld_rd=Column(Float,default=0.0); net_worker_payout_rd=Column(Float); fiscal_rule_code=Column(String(60),default="PENDIENTE_CLASIFICACION"); tax_mode=Column(String(30),default="CONFIGURACION"); tax_calculated_at=Column(DateTime); status=Column(String(50),default="RETENIDO"); voucher_url=Column(Text); bank_account_id=Column(Integer); payment_method=Column(String(50)); release_otp=Column(String(10)); otp_verified=Column(Boolean,default=False); created_at=Column(DateTime,default=datetime.utcnow); released_at=Column(DateTime)
class Review(Base):
...
class ProcessEvent(Base):
 __tablename__="process_events"; id=Column(String,primary_key=True,default=generate_uuid); user_id=Column(String,ForeignKey("users.id"),nullable=False,index=True); process_type=Column(String(80),nullable=False,index=True); status=Column(String(30),nullable=False,index=True); title=Column(String(200),nullable=False); message=Column(Text,nullable=False); next_step=Column(Text); rejection_reason=Column(Text); correction=Column(Text); related_entity_id=Column(String,index=True); created_at=Column(DateTime,default=datetime.utcnow,nullable=False)
class Message(Base):
