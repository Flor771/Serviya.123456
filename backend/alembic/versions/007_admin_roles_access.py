...
def upgrade() -> None:
    ...
    # Existing ADMIN accounts remain administrative, but only the configured
    # super-admin identity receives unrestricted role-management privileges.
    op.execute("UPDATE users SET admin_role = 'ADMIN_OPERACIONES' WHERE role = 'ADMIN' AND (admin_role IS NULL OR admin_role = '')")
    superadmin_email = (os.getenv("SUPERADMIN_EMAIL") or "admin@serviya.do").strip().lower()
    op.execute(sa.text("UPDATE users SET admin_role = 'SUPER_ADMIN', active_role = 'ADMIN', is_active = TRUE WHERE role = 'ADMIN' AND lower(email) = :email").bindparams(email=superadmin_email))
