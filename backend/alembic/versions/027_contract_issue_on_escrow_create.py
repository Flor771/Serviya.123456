...
def upgrade() -> None:
    ...
    BEGIN
        IF NEW.status <> 'RETENIDO' THEN RETURN NEW; END IF;
        ...
        IF svc.id IS NULL OR client.id IS NULL OR worker.id IS NULL THEN RETURN NEW; END IF;
        ...
        payload = jsonb_build_object(
            ...
            'contract_number',contract_no,'issued_at',CURRENT_TIMESTAMP,'issued_by_admin_id','ADMIN_APPROVE_DEPOSIT',
        ...
        WHERE NOT EXISTS (SELECT 1 FROM notifications n WHERE n.user_id=client.id AND n.type='CONTRACT_ISSUED' AND n.related_entity_id=NEW.service_id);
        INSERT INTO notifications(id,user_id,title,message,type,related_entity_id,read,created_at)
        ...
        WHERE NOT EXISTS (SELECT 1 FROM notifications n WHERE n.user_id=worker.id AND n.type='CONTRACT_ISSUED' AND n.related_entity_id=NEW.service_id);
        RETURN NEW;
    END;
    $$ LANGUAGE plpgsql;
    """))
    ...
    """))
    # PostgreSQL fires UPDATE OF status even when assigning the same value.
    # The INSERT uses ON CONFLICT(service_id) DO NOTHING, making this idempotent.
    op.execute(sa.text("""
        UPDATE escrows e SET status='RETENIDO'
        WHERE e.status='RETENIDO'
          AND NOT EXISTS (SELECT 1 FROM digital_contracts dc WHERE dc.service_id=e.service_id)
    """))
