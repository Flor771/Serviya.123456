...
import {AdminRoleWindows} from '../src/components/AdminRoleWindows';
import {api,getAuthToken,removeAuthToken,setAuthToken} from '../src/services/api';
import '../src/index.css';

const BACKEND_API='https://serviya-com-odg.onrender.com/api/v1';
if(typeof window!=='undefined'){
 const nativeFetch=window.fetch.bind(window);
 window.fetch=(input:RequestInfo|URL,init?:RequestInit)=>{const raw=typeof input==='string'?input:input instanceof URL?input.toString():input.url;const target=raw.startsWith('/api/v1')?`${BACKEND_API}${raw.slice('/api/v1'.length)}`:raw;return nativeFetch(target,init)};
}
...
function AdminApp(){
 ...
 if(authorized)return <AdminRoleWindows/>;
