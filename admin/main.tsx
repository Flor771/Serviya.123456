...
// The admin site is hosted separately from the API. Route its relative /api/v1
// requests to the real SERVIYA backend before React/API clients are initialized.
const BACKEND_API='https://serviya-com-odg.onrender.com/api/v1';
if(typeof window!=='undefined'){
 const nativeFetch=window.fetch.bind(window);
 window.fetch=(input:RequestInfo|URL,init?:RequestInit)=>{
  const raw=typeof input==='string'?input:input instanceof URL?input.toString():input.url;
  const target=raw.startsWith('/api/v1')?`${BACKEND_API}${raw.slice('/api/v1'.length)}`:raw;
