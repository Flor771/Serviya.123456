...
class TestRegistrationAndAuth(unittest.TestCase):
    ...
    def test_5_trabajador_registration_without_bank(self):
        ...
        self.assertIsInstance(wallet.id, int, "wallet.id MUST be an integer generated by database sequence")
        ...
        self.assertEqual(wallet.total_earnings, 0.0)
        self.assertEqual(wallet.total_commissions, 0.0)
        self.assertEqual(wallet.total_withdrawn, 0.0)
        db.close()
    ...
    def test_7_wallet_production_schema_compatibility(self):
        col_names = [c.name for c in Wallet.__table__.columns]
        self.assertIn("id", col_names)
        self.assertIn("worker_id", col_names)
        self.assertIn("available_balance", col_names)
        self.assertIn("pending_custody_balance", col_names)
        self.assertIn("total_earnings", col_names)
        self.assertIn("total_commissions", col_names)
        self.assertIn("total_withdrawn", col_names)

        self.assertNotIn("updated_at", col_names, "updated_at must NOT exist in Wallet table")
        self.assertNotIn("user_id", col_names, "user_id column must NOT exist in Wallet table")
        self.assertNotIn("available_rd", col_names, "available_rd column must NOT exist in Wallet table")
        self.assertNotIn("escrow_rd", col_names, "escrow_rd column must NOT exist in Wallet table")

        id_col = Wallet.__table__.columns["id"]
