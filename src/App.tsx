...
const AppContent: React.FC = () => {
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-800 font-sans flex flex-col overflow-x-hidden">
    ...
    <main className="flex-1 max-w-7xl w-full min-w-0 mx-auto px-3 sm:px-6 lg:px-8 pt-4 sm:pt-6 pb-24 md:pb-12 overflow-x-hidden">
      ...
      {user && isWorker && activeTab === 'mis-servicios' && <section className="space-y-4"><div className="bg-white rounded-3xl p-6 border border-slate-200"><p className="text-emerald-700 text-xs font-bold uppercase">TRABAJADOR / TÉCNICO</p><h1 className="text-2xl font-black mt-1">Mis trabajos</h1><p className="text-sm text-slate-500 mt-1">Servicios que tienes asignados o en proceso.</p></div>{workerServices.length?<div className="grid grid-cols-1 md:grid-cols-2 gap-4">{workerServices.map(s=><div key={s.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm"><button onClick={()=>openService(s)} className="text-left w-full min-w-0"><h3 className="font-bold break-words">{s.title}</h3><p className="text-sm text-slate-500 mt-1 break-words">RD$ {Number(s.price_rd).toLocaleString()} • {s.status}</p></button><button onClick={()=>setPhotoService(s)} className="mt-3 inline-flex items-center text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-2 rounded-xl">Subir fotos del trabajo</button></div>)}</div>:<div className="bg-white rounded-2xl p-6 text-sm text-slate-500 border border-slate-200">No tienes trabajos asignados todavía.</div>}</section>}
