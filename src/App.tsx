...
import { ExpressMandadoModal } from './components/ExpressMandadoModal';
...
const AppContent: React.FC = () => {
  ...
  const [expressMode, setExpressMode] = useState<'EXPRESS'|'MANDADO'|null>(null);
  ...
  useEffect(() => {
    const handleNavigate = (event: Event) => {
      ...
      if (destination === 'applications') setActiveTab(isWorker ? 'postulaciones' : 'mis-publicaciones');
        ...
        const isClientPostulation = isClient && (notificationType.includes('APPLICATION') || notificationType.includes('POSTUL'));
        setActiveTab(isClientPostulation ? 'mis-publicaciones' : (detail.tab || (isWorker ? 'mis-servicios' : 'mis-trabajos')));
        if (serviceId) { const service = services.find(s => String(s.id) === serviceId); if (service) window.setTimeout(() => setSelectedService(service), 120); }
      }
    };
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-800 font-sans flex flex-col overflow-x-hidden">
    ...
    <main className="flex-1 max-w-7xl w-full min-w-0 mx-auto px-3 sm:px-6 lg:px-8 pt-4 sm:pt-6 pb-24 md:pb-12 overflow-x-hidden">
      {!user && activeTab === 'inicio' && <ModernLanding services={services} onSelectCategory={() => setActiveTab('buscar')} onSelectService={openService} onOpenPublish={() => openAuth('login','CLIENTE')} onNavigateTab={setActiveTab} />}
      {user && !isWorker && activeTab === 'inicio' && <div className="space-y-5"><ModernLanding services={services} onSelectCategory={() => setActiveTab('buscar')} onSelectService={openService} onOpenPublish={() => setShowPublishModal(true)} onNavigateTab={setActiveTab} /><section className="grid grid-cols-1 sm:grid-cols-3 gap-3"><button onClick={() => setActiveTab('mis-publicaciones')} className="rounded-2xl bg-white border border-slate-200 p-4 text-left shadow-sm hover:border-blue-300"><p className="text-[10px] uppercase tracking-widest font-black text-blue-600">Mi actividad</p><h3 className="font-black mt-1">Mis publicaciones</h3><p className="text-xs text-slate-500 mt-1">{clientServices.length} registradas</p></button><button onClick={() => setActiveTab('mis-trabajos')} className="rounded-2xl bg-white border border-slate-200 p-4 text-left shadow-sm hover:border-blue-300"><p className="text-[10px] uppercase tracking-widest font-black text-blue-600">Trabajos</p><h3 className="font-black mt-1">Mis trabajos</h3><p className="text-xs text-slate-500 mt-1">{clientJobs.length} activos o finalizados</p></button><button onClick={() => setActiveTab('trabajadores')} className="rounded-2xl bg-white border border-slate-200 p-4 text-left shadow-sm hover:border-blue-300"><p className="text-[10px] uppercase tracking-widest font-black text-blue-600">Profesionales</p><h3 className="font-black mt-1">Buscar trabajadores</h3><p className="text-xs text-slate-500 mt-1">Encuentra personal capacitado</p></button></section></div>}
{user && !isWorker && activeTab === 'mis-publicaciones' && <section className="space-y-4"><div className="bg-white rounded-3xl p-6 border border-slate-200"><p className="text-blue-700 text-xs font-black uppercase">CLIENTE</p><div className="flex items-center justify-between gap-3"><div><h1 className="text-2xl font-black mt-1">Mis publicaciones</h1><p className="text-sm text-slate-500 mt-1">Aquí aparecen los servicios que tú publicaste y las postulaciones recibidas.</p></div><span className="shrink-0 text-xs font-bold text-slate-500">{clientServices.length}</span></div></div>{clientServices.length?<div className="grid grid-cols-1 md:grid-cols-2 gap-4">{clientServices.map(s=><article key={s.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm"><button onClick={()=>openService(s)} className="w-full text-left"><p className="text-xs text-blue-700 font-bold">{s.category_name}</p><h2 className="font-black mt-1 break-words">{s.title}</h2><p className="text-sm text-slate-500 mt-1 break-words">RD$ {Number(s.negotiated_price_rd||s.price_rd||0).toLocaleString()} • {String(s.status).replaceAll('_',' ')}</p><p className="text-xs text-slate-500 mt-2">Toca para ver la publicación y gestionar las propuestas.</p></button>{s.images?.length>0&&<button onClick={()=>setPhotoService(s)} className="mt-3 text-xs font-bold text-blue-700">Ver fotos ({s.images.length})</button>}</article>)}</div>:<div className="bg-white rounded-2xl p-6 border border-slate-200 text-sm text-slate-500">Todavía no has publicado servicios.</div>}</section>}
      {user && !isWorker && activeTab === 'mis-trabajos' && <section className="space-y-4"><div className="bg-white rounded-3xl p-6 border border-slate-200"><p className="text-blue-700 text-xs font-bold uppercase">CLIENTE</p><div className="flex items-start justify-between gap-3"><div><h1 className="text-2xl font-black mt-1">Mis trabajos</h1><p className="text-sm text-slate-500 mt-1">Servicios con depósito confirmado por administración y trabajo activo o finalizado.</p></div><button type="button" onClick={()=>window.dispatchEvent(new CustomEvent('serviya:navigate',{detail:{destination:'revisitas'}}))} className="shrink-0 rounded-xl bg-indigo-600 text-white px-3 py-2 text-xs font-black">Garantía / Revisita</button></div></div>{clientJobs.length?<div className="grid grid-cols-1 md:grid-cols-2 gap-4">{clientJobs.map(s=><div key={s.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm"><button onClick={()=>openService(s)} className="text-left w-full min-w-0"><p className="text-xs text-blue-700 font-bold">{s.category_name}</p><h3 className="font-bold mt-1 break-words">{s.title}</h3><p className="text-sm text-slate-500 mt-1 break-words">RD$ {Number(s.negotiated_price_rd||s.price_rd||0).toLocaleString()} • {s.status}</p><p className="text-xs text-emerald-700 font-bold mt-2">Depósito confirmado por administración</p></button><div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2"><button onClick={()=>s.worker_id&&openChat(s.id,s.worker_id)} className="w-full inline-flex items-center justify-center gap-1 py-2.5 rounded-xl bg-blue-50 border border-blue-200 text-blue-700 text-xs font-bold"><MessageSquare className="w-4 h-4"/> Mensaje al trabajador</button>{s.worker_id&&['EN_PROGRESO','FINALIZANDO','PENDIENTE_APROBACION'].includes(String(s.status))&&<button onClick={()=>setDisputeServiceId(String(s.id))} className="w-full inline-flex items-center justify-center gap-1 py-2.5 rounded-xl bg-red-50 border border-red-200 text-red-700 text-xs font-black">⚠️ Abrir disputa</button>}</div></div>)}</div>:<div className="bg-white rounded-2xl p-6 text-sm text-slate-500 border border-slate-200">Todavía no tienes trabajos activos. El trabajo aparecerá aquí después de que el cliente envíe el voucher y administración confirme que el dinero llegó.</div>}</section>}
      {user && !isWorker && activeTab === 'trabajadores' && <WorkersView />}
      {user && isWorker && activeTab === 'inicio' && <div className="space-y-6"><section className="rounded-3xl bg-slate-900 text-white p-6 sm:p-8 shadow-xl"><p className="text-emerald-300 text-xs font-bold uppercase tracking-widest">SERVIYA • TRABAJADOR / TÉCNICO</p><h1 className="text-2xl sm:text-3xl font-black mt-1">Encuentra trabajos y administra tus servicios.</h1><p className="text-slate-300 text-sm mt-2 max-w-2xl">Postúlate a servicios, gestiona los trabajos que aceptes y cobra mediante la Billetera SERVIYA.</p><div className="flex flex-wrap gap-2 mt-5"><button onClick={() => setActiveTab('buscar')} className="bg-blue-600 px-4 py-2.5 rounded-xl text-sm font-bold">Buscar trabajos</button><button onClick={() => setActiveTab('buscar')} className="bg-amber-500 text-white px-4 py-2.5 rounded-xl text-sm font-black">⚡ Express cerca de mí</button><button onClick={() => setActiveTab('buscar')} className="bg-emerald-600 text-white px-4 py-2.5 rounded-xl text-sm font-black">🛵 Mandados</button><button onClick={() => setActiveTab('mis-servicios')} className="bg-slate-700 px-4 py-2.5 rounded-xl text-sm font-bold">Mis trabajos</button><button onClick={() => setActiveTab('billetera')} className="bg-emerald-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold">Mi billetera</button></div></section><section className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div className="rounded-3xl bg-amber-50 border border-amber-200 p-5"><p className="text-amber-700 text-xs font-black uppercase">⚡ Oportunidades rápidas</p><h2 className="text-xl font-black text-slate-900 mt-1">SERVIYA EXPRESS</h2><p className="text-sm text-slate-600 mt-1">Encuentra trabajos por horas y tareas urgentes.</p><button onClick={() => setActiveTab('buscar')} className="mt-4 rounded-xl bg-amber-500 text-white px-4 py-2 text-xs font-black">Ver Express</button></div><div className="rounded-3xl bg-emerald-50 border border-emerald-200 p-5"><p className="text-emerald-700 text-xs font-black uppercase">🛵 Diligencias</p><h2 className="text-xl font-black text-slate-900 mt-1">SERVIYA MANDADOS</h2><p className="text-sm text-slate-600 mt-1">Compra, recoge, lleva o entrega. Busca oportunidades.</p><button onClick={() => setActiveTab('buscar')} className="mt-4 rounded-xl bg-emerald-600 text-white px-4 py-2 text-xs font-black">Ver Mandados</button></div></section></div>}
    ...
    {expressMode&&user&&!isWorker&&<ExpressMandadoModal mode={expressMode} onClose={()=>setExpressMode(null)} onSuccess={()=>{setExpressMode(null);fetchServices();}} />}
  </div>;
};
