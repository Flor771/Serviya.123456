...
const AppContent: React.FC = () => {
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-800 font-sans flex flex-col">
    <Header onOpenAuth={(mode) => setAuthModalMode(mode)} onOpenPublish={() => setShowPublishModal(true)} onOpenWallet={() => setActiveTab('billetera')} onOpenNotifications={() => setShowNotificationsModal(true)} onOpenMessages={() => setChatParams({ serviceId: '', receiverId: '' })} onOpenProfile={() => setActiveTab('perfil')} onOpenVerification={() => setShowVerificationModal(true)} onOpenDisputes={() => setDisputeServiceId('')} onOpenPolicies={() => setShowPoliciesModal(true)} onNavigateTab={(tab) => setActiveTab(tab)} activeTab={activeTab} />
    ...
    {user && <BottomNav activeTab={activeTab} onNavigateTab={setActiveTab} onOpenPublish={() => setShowPublishModal(true)} onOpenWallet={() => setActiveTab('billetera')} onOpenProfile={() => setActiveTab('perfil')} onOpenAuth={(mode) => setAuthModalMode(mode)} />}
