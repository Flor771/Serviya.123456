...
const AppContent: React.FC = () => {
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-800 font-sans flex flex-col">
    ...
    <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-20 md:pb-12">
      {!user && activeTab === 'inicio' && <>
        <section className="mb-6 rounded-3xl bg-white border border-slate-200 shadow-sm p-5 sm:p-7">
          <div className="text-center mb-5"><p className="text-xs font-black uppercase tracking-widest text-blue-600">SERVIYA</p><h2 className="text-2xl sm:text-3xl font-black text-slate-900 mt-1">¿Cómo vas a usar SERVIYA?</h2><p className="text-sm text-slate-500 mt-2">Elige tu rol para entrar a la experiencia correspondiente.</p></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button onClick={() => openAuth('login','CLIENTE')} className="text-left rounded-2xl border-2 border-blue-200 bg-blue-50 hover:border-blue-500 hover:bg-blue-100 p-5 transition"><p className="text-xs font-black text-blue-700 uppercase">ROL CLIENTE</p><h3 className="text-xl font-black text-slate-900 mt-1">Soy Cliente</h3><p className="text-sm text-slate-600 mt-2">Publicar servicios, buscar trabajadores, contratar, pagar mediante Custodia SERVIYA y confirmar trabajos.</p><span className="inline-block mt-4 bg-blue-600 text-white px-4 py-2 rounded-xl text-xs font-bold">Ingresar como Cliente</span></button>
            <button onClick={() => openAuth('login','TRABAJADOR')} className="text-left rounded-2xl border-2 border-emerald-200 bg-emerald-50 hover:border-emerald-500 hover:bg-emerald-100 p-5 transition"><p className="text-xs font-black text-emerald-700 uppercase">ROL TRABAJADOR / TÉCNICO</p><h3 className="text-xl font-black text-slate-900 mt-1">Soy Trabajador / Técnico</h3><p className="text-sm text-slate-600 mt-2">Buscar trabajos, postularse, gestionar trabajos asignados, enviar evidencia y solicitar retiros.</p><span className="inline-block mt-4 bg-emerald-600 text-white px-4 py-2 rounded-xl text-xs font-bold">Ingresar como Trabajador / Técnico</span></button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3"><button onClick={() => openAuth('register','CLIENTE')} className="text-xs font-bold text-blue-700 py-2">Crear cuenta de Cliente</button><button onClick={() => openAuth('register','TRABAJADOR')} className="text-xs font-bold text-emerald-700 py-2">Crear cuenta de Trabajador / Técnico</button></div>
        </section>
        <LandingSection services={services} workers={[]} onSelectCategory={() => setActiveTab('buscar')} onSelectService={openService} onOpenPublish={() => openAuth('login','CLIENTE')} onNavigateTab={setActiveTab} />
      </>}
      {user && !isWorker && activeTab === 'inicio' && <div className="space-y-6"><section className="rounded-3xl bg-slate-900 text-white p-6 sm:p-8 shadow-xl"><p className="text-blue-300 text-xs font-bold uppercase tracking-widest">SERVIYA • CLIENTE</p><h1 className="text-2xl sm:text-3xl font-black mt-1">Encuentra al profesional que necesitas.</h1><p className="text-slate-300 text-sm mt-2 max-w-2xl">Publica tu servicio, recibe propuestas y paga con Custodia SERVIYA. Tus fondos permanecen protegidos hasta que confirmes el trabajo.</p><div className="flex flex-wrap gap-2 mt-5"><button onClick={() => setActiveTab('buscar')} className="bg-blue-600 hover:bg-blue-500 px-4 py-2.5 rounded-xl text-sm font-bold">Buscar servicios</button><button onClick={() => setShowPublishModal(true)} className="bg-white text-slate-900 px-4 py-2.5 rounded-xl text-sm font-bold">Publicar servicio</button></div></section><section><div className="flex items-center justify-between mb-3"><h2 className="font-black text-lg">Mis servicios</h2><span className="text-xs text-slate-500">{clientServices.length} registrados</span></div>{clientServices.length ? <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{clientServices.map(s => <div key={s.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm"><button onClick={() => openService(s)} className="text-left w-full"><p className="text-xs text-blue-700 font-bold">{s.category_name}</p><h3 className="font-bold mt-1">{s.title}</h3><p className="text-sm text-slate-500 mt-1">RD$ {Number(s.price_rd).toLocaleString()} • {s.status}</p></button>{s.images?.length>0 && <button onClick={()=>setPhotoService(s)} className="mt-3 text-xs font-bold text-blue-700 hover:underline">📷 Ver fotos del servicio ({s.images.length})</button>}</div>)}</div> : <div className="bg-white rounded-2xl p-6 border border-slate-200 text-sm text-slate-500">Todavía no has publicado servicios.</div>}</section></div>}
      ...
      {user && isWorker && activeTab === 'mis-servicios' && <section className="space-y-4"><div className="bg-white rounded-3xl p-6 border border-slate-200"><p className="text-emerald-700 text-xs font-bold uppercase">TRABAJADOR / TÉCNICO</p><h1 className="text-2xl font-black mt-1">Mis trabajos</h1><p className="text-sm text-slate-500 mt-1">Servicios que tienes asignados o en proceso.</p></div>{workerServices.length ? <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{workerServices.map(s => <div key={s.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm"><button onClick={() => openService(s)} className="text-left w-full"><h3 className="font-bold">{s.title}</h3><p className="text-sm text-slate-500 mt-1">RD$ {Number(s.price_rd).toLocaleString()} • {s.status}</p></button><div className="flex flex-wrap gap-3 mt-3"><button onClick={()=>setPhotoService(s)} className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-2 rounded-xl">📷 Subir fotos del trabajo</button>{s.images?.length>0 && <button onClick={()=>setPhotoService(s)} className="text-xs font-bold text-blue-700 px-2">Ver fotos del cliente ({s.images.length})</button>}</div></div>)}</div> : <div className="bg-white rounded-2xl p-6 text-sm text-slate-500 border border-slate-200">No tienes trabajos asignados todavía.</div>}</section>}
      {user && activeTab === 'billetera' && <WalletView />}
      {user && activeTab === 'perfil' && <ProfileView onOpenVerification={() => setShowVerificationModal(true)} />}
    </main>
    {user && <BottomNav activeTab={activeTab} onNavigateTab={setActiveTab} onOpenPublish={() => setShowPublishModal(true)} onOpenWallet={() => setActiveTab('billetera')} onOpenProfile={() => setActiveTab('perfil')} onOpenAuth={(mode) => openAuth(mode, authRole)} />}
    {selectedService && <ServiceDetailModal service={selectedService} onClose={() => setSelectedService(null)} onRefresh={fetchServices} onOpenChat={openChat} onOpenReview={(sId,tId)=>setReviewParams({serviceId:sId,targetUserId:tId})} onOpenDispute={sId=>setDisputeServiceId(sId)} />}
