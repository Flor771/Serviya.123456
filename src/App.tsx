...
const AppContent: React.FC = () => {
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-800 font-sans flex flex-col overflow-x-hidden">
    ...
    <main className="flex-1 max-w-7xl w-full min-w-0 mx-auto px-3 sm:px-6 lg:px-8 pt-4 sm:pt-6 pb-24 md:pb-12 overflow-x-hidden">
      ...
      {user && isWorker && activeTab === 'inicio' && <div className="space-y-6"><section className="rounded-3xl bg-slate-900 text-white p-6 sm:p-8 shadow-xl"><p className="text-emerald-300 text-xs font-bold uppercase tracking-widest">SERVIYA • TRABAJADOR / TÉCNICO</p><h1 className="text-2xl sm:text-3xl font-black mt-1">Encuentra trabajos y administra tus servicios.</h1><p className="text-slate-300 text-sm mt-2 max-w-2xl">Postúlate a servicios, gestiona los trabajos que aceptes y cobra mediante la Billetera SERVIYA.</p><div className="flex flex-wrap gap-2 mt-5"><button onClick={() => setActiveTab('buscar')} className="bg-blue-600 px-4 py-2.5 rounded-xl text-sm font-bold">Buscar trabajos</button><button onClick={() => setActiveTab('buscar')} className="bg-amber-500 text-white px-4 py-2.5 rounded-xl text-sm font-black">⚡ Express cerca de mí</button><button onClick={() => setActiveTab('buscar')} className="bg-emerald-600 text-white px-4 py-2.5 rounded-xl text-sm font-black">🛵 Mandados</button><button onClick={() => setActiveTab('mis-servicios')} className="bg-slate-700 px-4 py-2.5 rounded-xl text-sm font-bold">Mis trabajos</button><button onClick={() => setActiveTab('billetera')} className="bg-emerald-600 px-4 py-2.5 rounded-xl text-sm font-bold">Mi billetera</button></div></section></div>}
