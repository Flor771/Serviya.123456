...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    const handleNavigate = (event: Event) => {
      ...
      // Close every app-level modal before opening the requested destination.
      ...
      setDisputeServiceId(null);
      setReviewParams(null);
      setWalletEscrowParams(null);
      ...
      if (destination === 'applications') {
        ...
        if (serviceId && user) {
          ...
          const receiverId = service ? (String(service.client_id) === String(user.id) ? String(service.worker_id || '') : String(service.client_id || '')) : '';
          setChatParams({ serviceId, receiverId });
        } else {
          setChatParams({ serviceId: '', receiverId: '' });
        }
      } else if (destination === 'wallet') {
        setActiveTab('billetera');
      } else if (destination === 'dispute') {
        setActiveTab('mis-trabajos');
        if (serviceId) setDisputeServiceId(serviceId);
      } else {
        setActiveTab(detail.tab || (isWorker ? 'mis-servicios' : 'mis-trabajos'));
        if (serviceId) {
          const service = services.find(s => String(s.id) === serviceId);
          if (service) window.setTimeout(() => setSelectedService(service), 120);
        }
      }
    };
    const handleWalletEscrow = (event: Event) => {
      const detail = (event as CustomEvent).detail || {};
      if (!detail.serviceId || !detail.amount) return;
      setShowNotificationsModal(false);
      setSelectedService(null);
      setChatParams(null);
      setShowPublishModal(false);
      ...
      setReviewParams(null);
      setActiveTab('billetera');
      setWalletEscrowParams({ serviceId: String(detail.serviceId), amount: Number(detail.amount) });
    };
    window.addEventListener('serviya:navigate', handleNavigate);
    window.addEventListener('serviya:open-wallet-escrow', handleWalletEscrow);
    return () => {
      window.removeEventListener('serviya:navigate', handleNavigate);
      window.removeEventListener('serviya:open-wallet-escrow', handleWalletEscrow);
    };
  }, [services, user, isWorker]);
  ...
  const clientJobs = clientServices.filter(s => Boolean(s.worker_id));
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-800 font-sans flex flex-col overflow-x-hidden">
    ...
    <main className="flex-1 max-w-7xl w-full min-w-0 mx-auto px-3 sm:px-6 lg:px-8 pt-4 sm:pt-6 pb-24 md:pb-12 overflow-x-hidden">
      ...
      {user && !isWorker && activeTab === 'mis-trabajos' && <section className="space-y-4"><div className="bg-white rounded-3xl p-6 border border-slate-200"><p className="text-blue-700 text-xs font-bold uppercase">CLIENTE</p><h1 className="text-2xl font-black mt-1">Mis trabajos</h1><p className="text-sm text-slate-500 mt-1">Servicios que publicaste y para los que ya tienes un trabajador seleccionado o un trabajo en proceso.</p></div>{clientJobs.length?<div className="grid grid-cols-1 md:grid-cols-2 gap-4">{clientJobs.map(s=><div key={s.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm"><button onClick={()=>openService(s)} className="text-left w-full min-w-0"><p className="text-xs text-blue-700 font-bold">{s.category_name}</p><h3 className="font-bold mt-1 break-words">{s.title}</h3><p className="text-sm text-slate-500 mt-1 break-words">RD$ {Number(s.negotiated_price_rd||s.price_rd||0).toLocaleString()} • {s.status}</p><p className="text-xs text-emerald-700 font-bold mt-2">Trabajador asignado</p></button><button onClick={()=>s.worker_id&&openChat(s.id,s.worker_id)} className="mt-3 w-full inline-flex items-center justify-center gap-1 py-2.5 rounded-xl bg-blue-50 border border-blue-200 text-blue-700 text-xs font-bold"><MessageSquare className="w-4 h-4"/> Mensaje al trabajador</button></div>)}</div>:<div className="bg-white rounded-2xl p-6 text-sm text-slate-500 border border-slate-200">Todavía no tienes trabajos contratados. Cuando selecciones un trabajador en una de tus publicaciones, aparecerá aquí.</div>}</section>}
      ...
      {user && isWorker && activeTab === 'postulaciones' && <ApplicationsView onOpenService={openService} />}
      ...
      {user && isWorker && activeTab === 'mis-servicios' && <section className="space-y-4"><div className="bg-white rounded-3xl p-6 border border-slate-200"><p className="text-emerald-700 text-xs font-bold uppercase">TRABAJADOR / TÉCNICO</p><h1 className="text-2xl font-black mt-1">Mis trabajos</h1><p className="text-sm text-slate-500 mt-1">Servicios que tienes asignados o en proceso.</p></div>{workerServices.length?<div className="grid grid-cols-1 md:grid-cols-2 gap-4">{workerServices.map(s=><div key={s.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm"><button onClick={()=>openService(s)} className="text-left w-full min-w-0"><h3 className="font-bold break-words">{s.title}</h3><p className="text-sm text-slate-500 mt-1 break-words">RD$ {Number(s.negotiated_price_rd||s.price_rd||0).toLocaleString()} • {s.status}</p></button><button onClick={()=>setPhotoService(s)} className="mt-3 inline-flex items-center text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-2 rounded-xl">Subir fotos del trabajo</button></div>)}</div>:<div className="bg-white rounded-2xl p-6 text-sm text-slate-500 border border-slate-200">No tienes trabajos asignados todavía.</div>}</section>}
      {user && activeTab === 'billetera' && <WalletView />}
      {user && activeTab === 'perfil' && <ProfileView onOpenVerification={() => setShowVerificationModal(true)} />}
    </main>
    {user&&<BottomNav activeTab={activeTab} onNavigateTab={(tab)=>{setSelectedService(null);setChatParams(null);setActiveTab(tab)}} onOpenPublish={()=>setShowPublishModal(true)} onOpenWallet={()=>{setSelectedService(null);setActiveTab('billetera')}} onOpenProfile={()=>{setSelectedService(null);setActiveTab('perfil')}} onOpenAuth={(mode)=>openAuth(mode,authRole)} onOpenMessages={()=>{setSelectedService(null);setChatParams({serviceId:'',receiverId:''})}} onOpenNotifications={()=>setShowNotificationsModal(true)} onOpenVerification={()=>setShowVerificationModal(true)} onOpenDisputes={()=>setDisputeServiceId('')} onOpenPolicies={()=>setShowPoliciesModal(true)} />}
    ...
    {selectedService&&user&&selectedService.status!=='TRABAJADOR_SELECCIONADO'&&<WorkFlowActions service={selectedService} onRefresh={fetchServices} onOpenChat={openChat} />}
