...
import { X, Lock, Mail, User as UserIcon, Phone, MapPin, Briefcase } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-slate-200 relative my-auto">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center gap-1 font-black text-2xl text-slate-900">
            SERVIYA<span className="text-red-600">.do</span> 🇩🇴
          </div>
          <p className="text-xs text-slate-500 mt-1">
            {mode === 'login' ? 'Ingresa a tu cuenta para continuar' : 'Crea tu cuenta de cliente o trabajador en RD'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          {mode === 'register' && (
            <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`py-2 text-xs font-bold rounded-xl border transition ${
                      role === 'CLIENTE' ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 text-slate-700 border-slate-200'
                    }`}
                  >
                    👤 Cliente (Contratar)
                  </button>
                  <button
                    ...
                    className={`py-2 text-xs font-bold rounded-xl border transition ${
                      role === 'TRABAJADOR' ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 text-slate-700 border-slate-200'
                    }`}
                  >
                    🛠️ Trabajador (Ofrecer)
                  </button>
                </div>
              </div>
              ...
              {/* Profession if Worker */}
              {role === 'TRABAJADOR' && (
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">Especialidad Principal *</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">Cédula RD</label>
                  <input
                    type="text"
                    value={cedula}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl text-xs shadow-lg transition disabled:opacity-50"
          >
            {loading ? 'Procesando...' : mode === 'login' ? 'Ingresar a mi Cuenta' : '🚀 Registrarme en SERVIYA.do'}
          </button>
        </form>

        <div className="mt-4 pt-3 border-t border-slate-100 text-center text-xs text-slate-500">
          {mode === 'login' ? (
            <p>¿No tienes cuenta? <button onClick={() => setMode('register')} className="text-blue-600 font-bold hover:underline">Regístrate gratis</button></p>
          ) : (
            <p>¿Ya tienes cuenta? <button onClick={() => setMode('login')} className="text-blue-600 font-bold hover:underline">Inicia sesión</button></p>
          )}
        </div>
