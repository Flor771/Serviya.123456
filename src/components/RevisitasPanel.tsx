...
export const RevisitasPanel: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const request = async () => {
    if (!selected) return;
    if (issue.trim().length < 3 || description.trim().length < 10) {
      setError('Indica el problema y una explicación de al menos 10 caracteres.');
      return;
    }
    setBusy(true); setError('');
    try {
      const r = await api.post<any>(`/completion/${selected.service_id}/warranty/revisit`, { issue: issue.trim(), description: description.trim() });
  ...
  const workerAction = async (serviceId: string, revisitId: string, action: 'schedule' | 'start' | 'complete') => {
    ...
    try {
      if (action === 'schedule') {
        ...
        if (notes.trim().length < 10) { setError('Explica la corrección realizada (mínimo 10 caracteres).'); return; }
        await api.post(`/completion/${serviceId}/warranty/revisits/${revisitId}/complete`, { resolution_notes: notes.trim() });
        setNotes('');
      }
      await load();
    } catch (e: any) {
