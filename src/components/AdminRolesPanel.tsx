    ...
    <section id="admin-configuration" className="bg-white p-6 rounded-3xl border-2 border-blue-100 shadow-sm space-y-6 scroll-mt-6">
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <form onSubmit={createAdmin} className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
          ...
          <p className="text-[11px] text-slate-500">El Super Administrador es único. Los demás administradores tienen su propia cuenta, contraseña y permisos por área.</p>
        ...
        <div className="space-y-3">
          ...
          {admins.map(a=><div key={a.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
            <div className="flex justify-between gap-3"><div><b>{a.first_name} {a.last_name}</b><p className="text-xs text-slate-500">{a.email} • {a.phone}</p><p className="text-[11px] font-bold text-blue-700 mt-1">{a.role_label || a.admin_role || 'Sin rol'}</p></div><span className={`text-[10px] font-black px-2 py-1 rounded-full ${a.is_active?'bg-emerald-100 text-emerald-700':'bg-slate-200 text-slate-500'}`}>{a.is_active?'ACTIVO':'INACTIVO'}</span></div>
