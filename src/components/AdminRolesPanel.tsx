...
export const AdminRolesPanel: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      const [data, roleData] = await Promise.all([api.get<any>('/admin/administrators'), api.get<any>('/admin/administrator-roles')]);
  ...
  const createAdmin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage('Administrador agregado. Su contraseña será la que utilice para entrar a su panel.');
    ...
    <section id="admin-configuration" className="bg-white p-6 rounded-3xl border-2 border-blue-100 shadow-sm space-y-6 scroll-mt-6">
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <form onSubmit={createAdmin} className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
          ...
          <p className="text-[11px] text-slate-500">El Super Administrador es único y tiene acceso directo a todos los paneles. Cada administrador nuevo recibe su propia contraseña.</p>
        ...
        <div className="space-y-3">
          ...
          {admins.map(a=><div key={a.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
            <div className="flex justify-between gap-3"><div><b>{a.first_name} {a.last_name}</b><p className="text-xs text-slate-500">{a.email} • {a.phone}</p><p className="text-[11px] font-bold text-blue-700 mt-1">{a.role_label || a.admin_role}</p></div><span className={`text-[10px] font-black px-2 py-1 rounded-full ${a.is_active?'bg-emerald-100 text-emerald-700':'bg-slate-200 text-slate-500'}`}>{a.is_active?'ACTIVO':'INACTIVO'}</span></div>
      ...
      <div className="p-3 rounded-xl bg-blue-50 border border-blue-100 text-[11px] text-blue-900"><b>Acceso:</b> el Super Administrador usa su sesión administrativa ya autenticada para entrar directamente a todas las áreas. Los administradores agregados tienen su propia cuenta y contraseña, y su rol define el área que deben gestionar.</div>
    </section>
  );
};
