...
export const LandingSection: React.FC<LandingSectionProps> = ({ services, workers, onSelectCategory, onSelectService, onOpenPublish, onNavigateTab }) => {
  ...
  return (
    <div className="space-y-10 sm:space-y-14 pb-12">
      ...
      <section className="space-y-5"><div className="flex items-end justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-widest text-blue-600">Explora</p><h2 className="mt-1 text-2xl sm:text-3xl font-black text-slate-900">Servicios para cada necesidad</h2><p className="mt-1 text-xs sm:text-sm text-slate-500">Desde oficios cotidianos hasta especialidades profesionales.</p></div><button onClick={() => onNavigateTab('buscar')} className="hidden sm:flex items-center gap-1 text-xs font-bold text-blue-600">Ver todos <ArrowRight className="w-4 h-4" /></button></div><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">{SERVICE_CATEGORIES.slice(0, 10).map(cat => <button key={cat.id} onClick={() => onSelectCategory(cat.id)} className="rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-blue-600">{categoryIcon(cat.icon)}</div><h3 className="mt-3 text-sm font-black text-slate-900">{cat.name}</h3><p className="mt-1 line-clamp-2 text-[11px] leading-relaxed text-slate-500">{cat.description}</p></button>)}</div></section>

      <section className="rounded-3xl bg-slate-950 p-6 sm:p-9 text-white shadow-xl"><div className="max-w-2xl"><p className="text-[11px] font-black uppercase tracking-widest text-blue-300">Simple y claro</p><h2 className="mt-2 text-2xl sm:text-3xl font-black">Así funciona SERVIYA</h2><p className="mt-2 text-sm text-slate-400">Publica → Encuentra → Acuerda → Custodia → Completa → Libera.</p></div><div className="mt-7 grid grid-cols-2 lg:grid-cols-6 gap-3">{[['01','Publica','Describe lo que necesitas.'],['02','Encuentra','Recibe y revisa propuestas.'],['03','Acuerda','Define precio y condiciones.'],['04','Custodia','El pago entra al flujo seguro.'],['05','Completa','El trabajador realiza el servicio.'],['06','Libera','Se aprueba y se paga al trabajador.']].map(([num,title,text]) => <div key={num} className="rounded-2xl border border-white/10 bg-white/[.04] p-4"><span className="text-[10px] font-black text-blue-300">{num}</span><h3 className="mt-2 text-sm font-black">{title}</h3><p className="mt-1 text-[11px] leading-relaxed text-slate-500">{text}</p></div>)}</div></section>

      <section className="grid grid-cols-1 md:grid-cols-3 gap-4"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><ShieldCheck className="w-6 h-6 text-blue-600" /><h3 className="mt-3 font-black text-slate-900">Trabajadores verificados</h3><p className="mt-1 text-xs leading-relaxed text-slate-500">La plataforma puede validar identidad y certificaciones para mostrar el distintivo de trabajador verificado.</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><LockKeyhole className="w-6 h-6 text-emerald-600" /><h3 className="mt-3 font-black text-slate-900">Pago en Custodia</h3><p className="mt-1 text-xs leading-relaxed text-slate-500">El flujo de custodia protege los fondos mientras el servicio se realiza y hasta la liberación correspondiente.</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><HeartHandshake className="w-6 h-6 text-amber-500" /><h3 className="mt-3 font-black text-slate-900">Confianza y seguimiento</h3><p className="mt-1 text-xs leading-relaxed text-slate-500">Mensajes, evidencias, disputas y garantía ayudan a mantener el servicio documentado de principio a fin.</p></div></section>
