...
export const LandingSection: React.FC<LandingSectionProps> = ({ services, workers, onSelectCategory, onSelectService, onOpenPublish, onNavigateTab }) => {
  ...
  const getCategoryImage = (cat: any) => {
    const key = String(cat.id || cat.name || '').toLowerCase().normalize('NFD').replace(/[\\u0300-\\u036f]/g, '').replace(/[^a-z0-9]/g, '');
    return categoryImages[key] || categoryImages[String(cat.name || '').toLowerCase().replace(/[^a-z0-9]/g, '')] || categoryImages.reparaciones;
  };

  const serviceImage = (service: Service) => {
    const key = String(service.category_name || '').toLowerCase().normalize('NFD').replace(/[\\u0300-\\u036f]/g, '').replace(/[^a-z0-9]/g, '');
    return categoryImages[key] || categoryImages.reparaciones;
  };

  const smallJobs = [
    { title: 'Instalar una lámpara', category: 'Electricidad', price: 'RD$1,000', image: categoryImages.electricidad },
    { title: 'Arreglar una llave de agua', category: 'Plomería', price: 'RD$1,500', image: categoryImages.plomeria },
    { title: 'Limpieza de apartamento', category: 'Limpieza', price: 'RD$1,500', image: categoryImages.limpieza },
    { title: 'Montaje de muebles', category: 'Reparaciones', price: 'Desde RD$800', image: categoryImages.reparaciones },
  ];

  const largeJobs = [
    { title: 'Remodelación completa', text: 'Coordina equipos y profesionales para renovar una vivienda.', price: 'Desde RD$250,000', image: 'https://images.unsplash.com/photo-1505691938895-1758d7feb511?auto=format&fit=crop&w=1200&q=80' },
    { title: 'Construcción', text: 'Encuentra profesionales para trabajos de mayor alcance.', price: 'Cotización', image: 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&w=1200&q=80' },
    { title: 'Pintura de proyecto', text: 'Interior, exterior, viviendas, locales y proyectos completos.', price: 'Cotización', image: categoryImages.pintura },
  ];
  ...
  return (
    <div className="space-y-8 sm:space-y-12 pb-12">
      <section className="relative overflow-hidden rounded-[2rem] bg-slate-950 text-white shadow-2xl border border-slate-800">
        <div className="absolute inset-0 opacity-25 bg-[radial-gradient(circle_at_85%_15%,rgba(37,99,235,.8),transparent_35%),radial-gradient(circle_at_10%_100%,rgba(16,185,129,.45),transparent_35%)]" />
        <div className="relative grid lg:grid-cols-[1.05fr_.95fr] gap-8 items-center p-5 sm:p-8 lg:p-12">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[10px] sm:text-xs font-black uppercase tracking-widest text-blue-200">
              <span className="h-2 w-2 rounded-full bg-emerald-400" /> SERVIYA
            </div>
            <h1 className="mt-5 text-3xl sm:text-5xl lg:text-6xl font-black leading-[1.03] tracking-tight">¿Qué necesitas hacer <span className="text-blue-300">hoy?</span></h1>
            <p className="mt-4 max-w-xl text-sm sm:text-base text-slate-300 leading-relaxed">Encuentra trabajadores y técnicos para trabajos pequeños, servicios normales y grandes proyectos.</p>
            <div className="mt-6 flex items-center gap-2 rounded-2xl bg-white p-2 shadow-xl">
              <Search className="w-5 h-5 text-slate-400 ml-2 shrink-0" />
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && onNavigateTab('buscar')} placeholder="¿Qué servicio necesitas?" className="min-w-0 flex-1 bg-transparent px-1 py-3 text-sm text-slate-900 outline-none" />
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button onClick={() => onNavigateTab('buscar')} className="rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-black">Necesito un trabajador</button>
              <button onClick={onOpenPublish} className="rounded-xl bg-white/10 border border-white/15 px-4 py-2.5 text-xs font-black">Quiero publicar un trabajo</button>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="relative overflow-hidden rounded-3xl h-44 sm:h-56"><img src={categoryImages.reparaciones} className="absolute inset-0 w-full h-full object-cover" alt="Profesional realizando reparaciones" /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 to-transparent" /><div className="absolute bottom-4 left-4"><p className="text-[10px] uppercase tracking-widest font-black text-blue-300">Trabajos</p><p className="font-black">Hazlo realidad</p></div></div>
            <div className="relative overflow-hidden rounded-3xl h-44 sm:h-56 mt-7"><img src={categoryImages.construccion} className="absolute inset-0 w-full h-full object-cover" alt="Profesionales trabajando en construcción" /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 to-transparent" /><div className="absolute bottom-4 left-4"><p className="text-[10px] uppercase tracking-widest font-black text-emerald-300">Proyectos</p><p className="font-black">Del pequeño al grande</p></div></div>
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="flex items-end justify-between gap-3 mb-4">
          <div><p className="text-[10px] font-black uppercase tracking-widest text-blue-600">Explora</p><h2 className="text-2xl sm:text-3xl font-black text-slate-900">¿Qué servicio necesitas?</h2></div>
          <button onClick={() => onNavigateTab('buscar')} className="text-xs font-black text-blue-600">Ver todos →</button>
        </div>
        <div className="flex gap-3 overflow-x-auto pb-2 snap-x">
          {SERVICE_CATEGORIES.slice(0, 10).map(cat => (
            <button key={cat.id} onClick={() => onSelectCategory(cat.id)} className="group min-w-[150px] sm:min-w-[180px] snap-start overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-sm text-left hover:border-blue-300 hover:shadow-md">
              <div className="relative h-24"><img src={getCategoryImage(cat)} className="w-full h-full object-cover transition duration-300 group-hover:scale-105" alt={cat.name} /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/70 to-transparent" /><div className="absolute bottom-2 left-2 text-white">{categoryIcon(cat.icon)}</div></div>
              <div className="p-3"><h3 className="text-sm font-black text-slate-900">{cat.name}</h3><p className="mt-1 line-clamp-2 text-[10px] text-slate-500">{cat.description}</p></div>
            </button>
          ))}
        </div>
      </section>
      ...
      <section>
        <div className="flex items-end justify-between mb-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-blue-600">Nivel 01</p><h2 className="text-2xl font-black text-slate-900">Trabajos simples</h2><p className="text-xs text-slate-500 mt-1">Soluciones rápidas para las necesidades del día a día.</p></div><button onClick={() => onNavigateTab('buscar')} className="text-xs font-black text-blue-600">Ver trabajos →</button></div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {smallJobs.map(job => <button key={job.title} onClick={() => onNavigateTab('buscar')} className="group overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-sm text-left hover:shadow-md hover:border-blue-300"><div className="relative h-32"><img src={job.image} className="w-full h-full object-cover group-hover:scale-105 transition" alt={job.title} /><span className="absolute top-2 left-2 rounded-lg bg-white/90 px-2 py-1 text-[9px] font-black text-slate-800">{job.category}</span></div><div className="p-3"><h3 className="text-sm font-black text-slate-900">{job.title}</h3><p className="mt-2 text-xs font-black text-emerald-600">{job.price}</p></div></button>)}
        </div>
      </section>

      <section className="rounded-[2rem] bg-slate-900 p-4 sm:p-6 lg:p-7 text-white">
        <div className="flex items-end justify-between gap-3 mb-5"><div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-300">Nivel 03</p><h2 className="text-2xl sm:text-3xl font-black">Grandes proyectos</h2><p className="mt-1 text-xs text-slate-400">También puedes publicar trabajos de gran alcance.</p></div><button onClick={onOpenPublish} className="rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-black">Publicar proyecto</button></div>
        <div className="grid md:grid-cols-3 gap-3">
          {largeJobs.map(job => <button key={job.title} onClick={onOpenPublish} className="group relative overflow-hidden rounded-2xl h-52 text-left"><img src={job.image} className="absolute inset-0 w-full h-full object-cover transition duration-500 group-hover:scale-105" alt={job.title} /><div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/20 to-transparent" /><div className="absolute bottom-0 p-4"><h3 className="text-lg font-black">{job.title}</h3><p className="mt-1 text-xs text-slate-300">{job.text}</p><p className="mt-2 text-xs font-black text-emerald-300">{job.price}</p></div></button>)}
        </div>
      </section>

      <section className="rounded-3xl bg-white border border-slate-200 p-4 sm:p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-blue-600">Encuentra lo que necesitas</p><h2 className="mt-1 text-xl sm:text-2xl font-black text-slate-900">Busca por servicio y provincia</h2></div><span className="text-xs text-slate-500">RD$ · República Dominicana</span></div>
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-2">
          <div className="sm:col-span-6 flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3"><Search className="w-5 h-5 text-slate-400" /><input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Plomero, pintura, limpieza..." className="w-full bg-transparent text-sm text-slate-800 outline-none" /></div>
          <div className="sm:col-span-4 flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3"><MapPin className="w-5 h-5 text-slate-400" /><select value={selectedProvince} onChange={e => setSelectedProvince(e.target.value)} className="w-full bg-transparent text-sm text-slate-800 outline-none"><option value="">Todas las provincias</option>{DOMINICAN_PROVINCES.map(p => <option key={p.name} value={p.name}>{p.name}</option>)}</select></div>
          <button onClick={() => onNavigateTab('buscar')} className="sm:col-span-2 rounded-xl bg-slate-900 px-4 py-3 text-sm font-extrabold text-white">Buscar</button>
        </div>
      </section>

      <section className="space-y-4">
        <div className="flex items-end justify-between"><div><p className="text-[10px] font-black uppercase tracking-widest text-blue-600">Oportunidades</p><h2 className="text-2xl font-black text-slate-900">Trabajos publicados recientemente</h2></div><button onClick={() => onNavigateTab('buscar')} className="text-xs font-black text-blue-600">Ver todos →</button></div>
        {services.length ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{services.slice(0, 6).map(s => <button key={s.id} onClick={() => onSelectService(s)} className="group overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-sm text-left hover:border-blue-300 hover:shadow-md"><div className="relative h-36"><img src={serviceImage(s)} className="w-full h-full object-cover group-hover:scale-105 transition" alt={s.title} /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/70 to-transparent" /><span className="absolute bottom-3 left-3 rounded-lg bg-white/90 px-2 py-1 text-[10px] font-black text-slate-800">{s.category_name}</span></div><div className="p-4"><div className="flex justify-between gap-2"><h3 className="font-black text-slate-900 line-clamp-2">{s.title}</h3><span className="shrink-0 text-sm font-black text-emerald-600">RD$ {Number(s.price_rd || 0).toLocaleString()}</span></div><p className="mt-2 line-clamp-2 text-xs text-slate-500">{s.description}</p><div className="mt-3 flex items-center gap-1 text-[11px] text-slate-500"><MapPin className="w-3.5 h-3.5" />{s.municipality}, {s.province}</div></div></button>)}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-500">Aquí aparecerán los trabajos publicados recientemente.</div>}
      </section>

      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-2xl overflow-hidden bg-white border border-slate-200 shadow-sm"><img src={categoryImages.limpieza} className="h-32 w-full object-cover" alt="Trabajadora realizando limpieza" /><div className="p-4"><ShieldCheck className="w-6 h-6 text-blue-600" /><h3 className="mt-2 font-black text-slate-900">Trabajadores verificados</h3><p className="mt-1 text-xs leading-relaxed text-slate-500">La plataforma puede validar identidad y certificaciones para mostrar el distintivo correspondiente.</p></div></div>
        <div className="rounded-2xl overflow-hidden bg-white border border-slate-200 shadow-sm"><img src={categoryImages.pintura} className="h-32 w-full object-cover" alt="Profesional pintando" /><div className="p-4"><LockKeyhole className="w-6 h-6 text-emerald-600" /><h3 className="mt-2 font-black text-slate-900">Pago en Custodia</h3><p className="mt-1 text-xs leading-relaxed text-slate-500">El flujo de custodia protege los fondos mientras el servicio se realiza y hasta la liberación correspondiente.</p></div></div>
        <div className="rounded-2xl overflow-hidden bg-white border border-slate-200 shadow-sm"><img src={categoryImages.tecnologia} className="h-32 w-full object-cover" alt="Técnico trabajando con tecnología" /><div className="p-4"><HeartHandshake className="w-6 h-6 text-amber-500" /><h3 className="mt-2 font-black text-slate-900">Confianza y seguimiento</h3><p className="mt-1 text-xs leading-relaxed text-slate-500">Mensajes, evidencias, disputas y garantía ayudan a mantener el servicio documentado.</p></div></div>
      </section>

      <section className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-8 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4"><div><div className="flex items-center gap-2"><Star className="w-5 h-5 text-amber-500" /><h2 className="text-xl sm:text-2xl font-black text-slate-900">Preguntas frecuentes y mini tutorial</h2></div><p className="mt-2 max-w-2xl text-xs sm:text-sm leading-relaxed text-slate-500">Esta guía es opcional. Úsala cuando tengas una duda sobre contratar, trabajar, pagar o resolver una situación.</p></div><div className="flex items-center gap-2 rounded-xl bg-blue-50 px-3 py-2 text-[11px] font-bold text-blue-700"><span className="h-2 w-2 rounded-full bg-blue-500" /> Consulta libre</div></div>
        ...
        <div className="mt-4 flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3"><Search className="w-4 h-4 text-slate-400" /><input value={faqSearch} onChange={e => { setFaqSearch(e.target.value); setOpenFaq(null); }} placeholder="Buscar una pregunta: pago, retiro, garantía, cancelación..." className="w-full bg-transparent text-xs text-slate-800 outline-none" /></div>
