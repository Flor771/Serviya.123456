  ...
  return (<>
    <div className={embedded ? 'w-full' : 'fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 z-[45] w-[calc(100%-1.5rem)] max-w-2xl'}>
      <div className="bg-white border border-slate-200 shadow-xl rounded-2xl p-3">
        ...
        {workerRoleCanFinish && <div className="space-y-3 mt-3"><div className="rounded-xl bg-blue-50 border border-blue-200 p-3 text-xs text-blue-900"><b>Custodia retenida.</b> El pago está protegido. Sube la evidencia y el resumen antes de enviar el trabajo terminado al cliente para revisión.</div><div className="rounded-2xl border-2 border-emerald-200 bg-emerald-50 p-3 space-y-2"><p className="text-xs font-black text-emerald-950">¿Terminaste el trabajo?</p><p className="text-[11px] text-emerald-800">La evidencia, el resumen y el envío a revisión se realizan en un solo paso.</p><button onClick={() => setShowPhotos(true)} disabled={busy} className="w-full inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm px-4 py-3 rounded-xl disabled:opacity-50"><Camera className="w-5 h-5" />Subir prueba y enviar trabajo terminado</button></div></div>}

        {clientCanApprove && <div className="rounded-2xl border-2 border-amber-300 bg-amber-50 p-4 space-y-3"><div className="flex items-start gap-2"><CheckCircle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" /><div><p className="text-sm font-black text-amber-950">Trabajo terminado: revisa la evidencia</p><p className="text-[11px] text-amber-900 mt-1">Revisa el resumen y las fotos. Si todo está correcto, confirma la finalización para enviar la solicitud de liberación a Administración.</p></div></div>{completion?.summary && <div className="rounded-xl bg-white border border-amber-200 p-3"><p className="text-[10px] uppercase font-black text-slate-500">Resumen del trabajador</p><p className="text-xs text-slate-800 mt-1 whitespace-pre-wrap">{completion.summary}</p></div>}{evidenceReady && <div><p className="text-[10px] uppercase font-black text-slate-500 mb-2">Fotos de evidencia ({completion.photos.length})</p><div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{completion.photos.map((photo: string, index: number) => <a key={index} href={photo} target="_blank" rel="noreferrer" className="block rounded-xl overflow-hidden border border-amber-200 bg-white"><img src={photo} alt={`Evidencia ${index + 1}`} className="w-full h-28 object-cover" /></a>)}</div></div>}<button onClick={approveCompletion} disabled={busy} className="w-full inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm px-3 py-3 rounded-xl disabled:opacity-50"><CheckCircle className="w-5 h-5" />{busy ? 'Registrando…' : 'Aceptar trabajo finalizado y enviar a Administración'}</button></div>}
