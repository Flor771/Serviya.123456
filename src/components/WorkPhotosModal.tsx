...
export const WorkPhotosModal: React.FC<Props> = ({ service, onClose, onRefresh }) => {
  ...
  const save = async () => {
    ...
    setSuccess('');
    try {
      const data = await api.post<{ photos: string[]; message: string; completion_submitted?: boolean }>(`/services/${service.id}/completion-photos`, { photos: newPhotos, summary: summary.trim() });
      setPhotos(data.photos || []);
      setNewPhotos([]);
      setSummary('');
