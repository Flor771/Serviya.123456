...
export const WorkPhotosModal: React.FC<Props> = ({ service, onClose, onRefresh }) => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await api.get<{ photos: string[] }>(`/services/${service.id}/completion-photos`);
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    setError('');
    if (photos.length + newPhotos.length + files.length > MAX_PHOTOS) {
      setError(`Puedes guardar hasta ${MAX_PHOTOS} fotos de finalización.`);
      return;
    }
    try {
      ...
      setError(err.message || 'No se pudo procesar una foto.');
    }
  };
  ...
  const save = async () => {
    ...
    if (summary.trim().length < 5) {
      setError('Escribe un resumen breve de al menos 5 caracteres para enviar el trabajo terminado al cliente.');
      return;
    }
    setSaving(true);
    setError('');
    setSuccess('');
    try {
      const data = await api.post<{ photos: string[]; message: string }>(`/services/${service.id}/completion-photos`, { photos: newPhotos, summary: summary.trim() });
      setPhotos(data.photos || []);
      setNewPhotos([]);
      setSuccess('✓ Evidencia enviada correctamente. El trabajo terminado fue enviado al cliente para revisión.');

      // The evidence upload and final submission are one user action.
      await api.post(`/completion/${service.id}/submit`, { summary: summary.trim() });
      setSummary('');
      setSuccess('✓ Éxito: evidencia y trabajo terminado enviados correctamente al cliente. El cliente ya puede revisar las fotos y aceptar la finalización.');
      onRefresh?.();
    } catch (err: any) {
      setError(err.message || 'No se pudo enviar la evidencia y finalizar el trabajo.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/70 backdrop-blur-sm p-4 overflow-y-auto">
    <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 relative my-8 max-h-[90vh] overflow-y-auto">
      ...
      {loading ? <p className="text-sm text-slate-500">Cargando evidencia…</p> : <>
        ...
        {isWorker && <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 space-y-3"><div className="flex items-center gap-2"><Camera className="w-5 h-5 text-emerald-700" /><div><h3 className="font-black text-sm text-slate-900">Evidencia del trabajo terminado</h3><p className="text-xs text-slate-600">Sube fotos reales del resultado. Al enviarlas, SERVIYA las guarda y automáticamente manda el trabajo terminado al cliente.</p></div></div><label className="inline-flex items-center gap-2 cursor-pointer bg-white border border-emerald-300 text-emerald-700 font-bold text-xs px-3 py-2.5 rounded-xl hover:bg-emerald-100"><ImageIcon className="w-4 h-4" />Subir fotos del trabajo<input type="file" accept="image/*" multiple className="hidden" disabled={saving || photos.length + newPhotos.length >= MAX_PHOTOS} onChange={e => { void handleFiles(e.target.files); e.currentTarget.value=''; }} /></label>{newPhotos.length > 0 && <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">{newPhotos.map((src, i) => <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-emerald-200"><img src={src} alt={`Nueva evidencia ${i + 1}`} className="w-full h-full object-cover" /><button type="button" onClick={() => setNewPhotos(prev => prev.filter((_, x) => x !== i))} className="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1"><X className="w-3 h-3" /></button></div>)}</div>}<textarea value={summary} onChange={e => setSummary(e.target.value)} rows={3} maxLength={1000} placeholder="Resumen del trabajo realizado (obligatorio para enviar al cliente)" className="w-full text-xs p-3 bg-white border border-emerald-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500" /><button type="button" onClick={save} disabled={saving || !newPhotos.length || summary.trim().length < 5} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm py-3 rounded-xl disabled:opacity-50">{saving ? 'Enviando evidencia y finalización…' : '✓ Enviar evidencia y trabajo terminado al cliente'}</button></div>}
        {!isWorker && <div className="p-4 rounded-2xl bg-blue-50 border border-blue-200 text-xs text-blue-800 font-semibold">Estas fotos fueron subidas por el técnico como evidencia. Revísalas antes de confirmar la finalización y enviar la solicitud a Administración.</div>}
      </>}
    </div>
  </div>;
};