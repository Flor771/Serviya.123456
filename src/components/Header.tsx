        ...
        <div className="w-full max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between min-h-14 sm:min-h-20 gap-1">
            ...
            <nav className="hidden md:flex items-center justify-center gap-1 flex-[2]">
              ...
              <div className="relative"><button onClick={() => setMoreOpen(v => !v)} className="px-3 py-2 rounded-xl text-sm font-semibold text-slate-300 hover:bg-slate-800">Más <ChevronDown className="inline w-3.5 h-3.5" /></button>{moreOpen && <div className="absolute left-0 mt-2 w-60 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-2 z-50">
            ...
            <div className="flex items-center justify-end gap-0.5 sm:gap-2 shrink-0">
              {user && <>
                ...
                <div className="relative"><button onClick={() => setProfileOpen(v => !v)} className="p-1 rounded-xl hover:bg-slate-800" title="Mi perfil" aria-label="Mi perfil">{user.avatar_url ? <img src={user.avatar_url} alt={`Foto de perfil de ${user.first_name || 'usuario'}`} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full object-cover border-2 border-blue-500 bg-slate-800" /> : <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full border-2 border-blue-500 bg-slate-800 flex items-center justify-center"><UserIcon className="w-5 h-5 text-slate-300" /></div>}</button>{profileOpen && <div className="absolute right-0 mt-2 w-60 max-w-[calc(100vw-1rem)] bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-2 z-50">
          ...
          {mobileMenuOpen && <div className="md:hidden border-t border-slate-800 py-2 max-h-[calc(100vh-3.5rem)] overflow-y-auto">
            ...
            </> : <><button onClick={() => { close(); onOpenAuth('login'); }} className="w-full text-left px-3 py-3 rounded-xl text-sm font-semibold">Ingresar</button><button onClick={() => { close(); onOpenAuth('register'); }} className="w-full text-left px-3 py-3 rounded-xl text-sm font-bold text-blue-400">Registrarme</button></>}</div>}
        </div>
      </header>
    </>
  );
