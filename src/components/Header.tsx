...
export const Header: React.FC<HeaderProps> = ({
  ...
  const navItems: [string, string][] = isWorker
    ? [['inicio', 'Inicio'], ['buscar', 'Buscar trabajos'], ['mis-servicios', 'Mis trabajos']]
    : [['inicio', 'Inicio'], ['buscar', 'Buscar servicios']];
    ...
    <header className="sticky top-0 z-40 bg-slate-900 text-white border-b border-slate-800 shadow-lg overflow-x-clip">
      ...
      <div className="w-full max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between min-h-14 sm:min-h-20 gap-1">
          <button
            onClick={() => { close(); onNavigateTab('inicio'); }}
            className="flex items-center gap-2 min-w-0 flex-1 text-left"
          >
            <div className="w-9 h-9 sm:w-11 sm:h-11 shrink-0 rounded-2xl bg-gradient-to-tr from-blue-900 via-blue-800 to-red-600 flex items-center justify-center shadow-md">
              <span className="text-lg sm:text-2xl font-black">S</span>
            </div>
            <div className="min-w-0">
              <span className="block text-lg sm:text-2xl font-black leading-none truncate">SERVIYA</span>
              <span className="hidden sm:block text-[10px] text-slate-400 mt-1 truncate">Trabajo • Confianza • Oportunidades</span>
            </div>
          </button>

          <nav className="hidden md:flex items-center justify-center gap-1 flex-[2]">
            {navItems.map(([tab, label]) => (
              <button key={tab} onClick={() => onNavigateTab(tab)} className={`px-3 py-2 rounded-xl text-sm font-semibold whitespace-nowrap ${activeTab === tab ? 'bg-blue-600' : 'text-slate-300 hover:bg-slate-800'}`}>
                {label}
              </button>
            ))}
            {!isWorker && (
              <button onClick={() => onNavigateTab('trabajadores')} className={`px-3 py-2 rounded-xl text-sm font-semibold whitespace-nowrap ${activeTab === 'trabajadores' ? 'bg-blue-600' : 'text-slate-300 hover:bg-slate-800'}`}>
                Trabajadores
              </button>
            )}
            <div className="relative">
              <button onClick={() => setMoreOpen(v => !v)} className="px-3 py-2 rounded-xl text-sm font-semibold text-slate-300 hover:bg-slate-800">
                Más <ChevronDown className="inline w-3.5 h-3.5" />
              </button>
              {moreOpen && (
                <div className="absolute left-0 mt-2 w-56 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-2 z-50">
          ...
          <div className="flex items-center justify-end gap-0.5 sm:gap-2 shrink-0">
            {user ? (
              <>
                ...
                <button onClick={onOpenNotifications} className="relative p-2 rounded-xl hover:bg-slate-800" title="Notificaciones">
              ...
              <>
                <button onClick={() => onOpenAuth('login')} className="hidden sm:block px-2 py-2 text-xs font-semibold">Ingresar</button>
                <button onClick={() => onOpenAuth('register')} className="hidden sm:block px-3 py-2 bg-blue-600 rounded-xl text-xs font-bold">Registrarme</button>
              </>
            )}
            <button onClick={() => setMobileMenuOpen(v => !v)} className="md:hidden p-2 rounded-xl hover:bg-slate-800" title="Menú">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-slate-800 py-2 max-h-[calc(100vh-3.5rem)] overflow-y-auto">
            {navItems.map(([tab, label]) => (
              <button key={tab} onClick={() => { close(); onNavigateTab(tab); }} className="w-full text-left px-3 py-3 rounded-xl text-sm font-semibold hover:bg-slate-800">
                <Search className="inline w-4 h-4 mr-2 text-blue-400" />{label}
              </button>
            ))}
            {!isWorker && (
              <button onClick={() => { close(); onNavigateTab('trabajadores'); }} className="w-full text-left px-3 py-3 rounded-xl text-sm font-semibold hover:bg-slate-800">
                <BriefcaseBusiness className="inline w-4 h-4 mr-2 text-emerald-400" />Trabajadores
              </button>
            )}
            {user ? (
              <>
              ...
              <>
                <button onClick={() => { close(); onOpenAuth('login'); }} className="w-full text-left px-3 py-3 rounded-xl text-sm font-semibold">Ingresar</button>
                <button onClick={() => { close(); onOpenAuth('register'); }} className="w-full text-left px-3 py-3 rounded-xl text-sm font-bold text-blue-400">Registrarme</button>
              </>
            )}
          </div>
        )}
