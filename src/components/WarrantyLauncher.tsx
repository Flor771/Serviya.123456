...
export const WarrantyLauncher:React.FC=()=>{
 const [open,setOpen]=useState(false); const [serviceId,setServiceId]=useState(''); const [contractNumber,setContractNumber]=useState(''); const [data,setData]=useState<WarrantyPayload|null>(null); const [loading,setLoading]=useState(false); const [error,setError]=useState(''); const [issue,setIssue]=useState(''); const [description,setDescription]=useState(''); const [sending,setSending]=useState(false); const [message,setMessage]=useState('');
 useEffect(()=>{
  const inject=()=>{
 ...
 const close=()=>{setOpen(false);setData(null);setError('');setMessage('');setIssue('');setDescription('');setServiceId('');};
  ...
  <div className="p-4 sm:p-5 space-y-4">
   ...
   {data&&<>
    <section className={`rounded-2xl border p-5 ${data.warranty.expired?'border-slate-200 bg-white':'border-emerald-200 bg-emerald-50'}`}>
      ...
      <div className="mt-3 text-xs text-slate-600 space-y-1"><p><b>Contrato:</b> {contractNumber||'—'}</p><p><b>Servicio:</b> {data.warranty.service_title||'—'}</p><p><b>Cliente:</b> {data.warranty.client_name||'—'}</p><p><b>Trabajador:</b> {data.warranty.worker_name||'—'}</p><p><b>Activada:</b> {fmt(data.warranty.activated_at)}</p></div>
    </section>
    {!data.warranty.expired&&<section className="bg-white rounded-2xl border border-slate-200 p-5"><div className="flex items-center gap-2"><RefreshCw className="w-5 h-5 text-blue-600"/><h3 className="font-black text-slate-900">Solicitar revisita</h3></div><p className="text-xs text-slate-500 mt-1">Si el trabajo presenta un problema durante la garantía, registra aquí la solicitud.</p><input value={issue} onChange={e=>setIssue(e.target.value)} placeholder="Problema a revisar" className="mt-4 w-full rounded-xl border border-slate-200 p-3 text-sm outline-none focus:border-blue-400"/><textarea value={description} onChange={e=>setDescription(e.target.value)} placeholder="Describe el problema con detalle (mínimo 10 caracteres)" rows={4} className="mt-2 w-full rounded-xl border border-slate-200 p-3 text-sm outline-none focus:border-blue-400"/><button disabled={sending||issue.trim().length<3||description.trim().length<10} onClick={request} className="mt-3 w-full rounded-xl bg-blue-600 text-white py-3 text-sm font-black disabled:opacity-50">{sending?'Registrando…':'Solicitar revisita'}</button></section>}
