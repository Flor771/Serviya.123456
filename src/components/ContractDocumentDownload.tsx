...
function openPrintableContract(card: HTMLElement) {
  ...
  if (!popup) {
    window.alert('El navegador bloqueó la ventana del contrato. Permite ventanas emergentes para descargarlo.');
    return;
  }

  const clone = card.cloneNode(true) as HTMLElement;
  clone.querySelectorAll('button').forEach(button => button.remove());
  clone.querySelectorAll('[data-serviya-contract-download]').forEach(node => node.remove());
...
export const ContractDocumentDownload: React.FC = () => {
  useEffect(() => {
    const inject = () => {
      ...
      button.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); openPrintableContract(card); });
