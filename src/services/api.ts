...
export function getAuthToken(): string | null { return localStorage.getItem('serviya_token'); }
export function setAuthToken(token: string) { localStorage.setItem('serviya_token', token); }
export function removeAuthToken() { localStorage.removeItem('serviya_token'); }
...
export async function apiFetch<T = any>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  else { try { const text = await response.text(); data = { message: text || `Error HTTP ${response.status}: ${response.statusText}` }; } catch { data = { message: `Error HTTP ${response.status}: ${response.statusText}` }; } }
  if (!response.ok) {
...
async function request<T>(endpoint: string, options: RequestInit = {}): Promise<T> { return apiFetch<T>(endpoint, options); }
...
export const api = {
  ...
  delete: <T>(endpoint: string, body?: any) => request<T>(endpoint, { method: 'DELETE', body: body === undefined ? undefined : JSON.stringify(body) })
};
